...
import { nextPrayer, usePrayerTimes } from "@/app/hook/usePrayerTimes";
...
export default function PrayerPanel() {
  ...
  const { data, isLoading } = usePrayerTimes(lat, lng);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const diffSeconds = Math.floor(
        (times[next].getTime() - Date.now()) / 1000,
      );
  ...
  if (!data || !times) return <p>Failed to load.</p>;
