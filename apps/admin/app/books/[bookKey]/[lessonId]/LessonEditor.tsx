...
import type { Book, ContentBlock, Lesson } from "@punluesunnah/shared-types";
import { Button } from "@/app/ui/button";
import { Card, CardContent } from "@/app/ui/card";
import { Input } from "@/app/ui/input";
import Field from "@/app/_components/Field";
import { saveLessonAction } from "@/app/_actions";
...
export default function LessonEditor({
  ...
  const onSubmit = (event: React.FormEvent) => {
    ...
    startSaving(async () => {
      const result = await saveLessonAction(book.key, lesson.id, {
        khmer_title: khmerTitle,
        arabic_title: arabicTitle,
        content: blocks,
      });
      if (result.ok) toast.success("បានរក្សាទុក");
