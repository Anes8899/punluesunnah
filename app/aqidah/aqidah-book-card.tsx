...
import type { AqidahBook } from "@/lib/aqidah-data";
...
interface AqidahBookCardProps {
  book: AqidahBook;
  index: number;
}

export default function AqidahBookCard({ book, index }: AqidahBookCardProps) {
  const accent = ACCENTS[index % ACCENTS.length];

  return (
    <Link href={`/aqidah/${book.id}`}>
      <Card
